...
<center>Escreva o título de sua página!<br/>Máximo 60 caracteres.</center><br/>
<div class="col-12">
<form style="margin: 20px 0" action="/paginas?" method="post" enctype="multipart/form-data">
<div class="input-group input-group-sm mb-3"><div class="input-group-prepend"><span class="input-group-text"><a href="/paginas.php?a=catemocoes"><i style="color: #555555" class="far fa-grin-alt"></i></a></span></div>
<input type="text" class="form-control" placeholder="Digite título da sua página!." name="post" max-length="60"><div class="input-group-append"><!--<span class="input-group-text youtube" data-toggle="collapse" href="#youtube"><i class="fab fa-youtube"></i></span>--><span class="input-group-text file-chooser"><i class="fas fa-paperclip"></i></span><button class="btn btn-secondary send-message-chat" type="submit"><i class="fas fa-paper-plane"></i></button></div></div><div class="collapse" id="youtube" style="margin-bottom: 1rem"><input type="text" class="form-control form-control-sm" placeholder="Link do YouTube" name="youtube"></div><input type="file" id="file" name="arquivo" style="display: none"></div></form>
<?
...
<div class="card card-left shadow-sm bg-white"><img width="120" src="/<?php echo $item->imagem==false?'semfoto.jpg':''.$item->imagem.'';?>"><div class="card-body"><a class="card-title" href="paginas.php?a=comentarios&id=<?php echo $item->id;?>"><?php echo textot($item->txt,$meuid,$on);?></a><p class="card-text"><b>Dono(a): <a href="/<?php echo gerarlogin($item->uid);?>"><?php echo gerarnome($item->uid);?></a></b><br/><?
if(perm($meuid)>0 or $item->uid==$meuid){?>
<span class="badge badge-danger badge-notification users-number" style="font-size: 10px"><a href="/paginas?a=excluir&id=<?php echo $item->id;?>"><font color="white">[apagar]</font></span></a></span><?}?></div></div>
<?php  ?>
</div></li>
<?php $i++; } } else {?>
...
<div align="center"><br/>
<?php if($numpag>1) { 
paginas('paginas',0,$numpag,$id,$pag);   
}
...
$mistake->exec("DELETE FROM curtidas WHERE msg='$id' and local='paginas'");
header("Location:/paginas.php?");
}
}
else 
...
?>
<div align="center"><a href="/paginas/catemocoes"><?php echo $imgsetavoltar;?> Voltar Emoções</a><br/>
...
if($a=='ultimos') { 
ativo($meuid,'Vendo últimos recados'); ?>
<?php

?>
<section class="container-fluid"><h2 class="title"><b>Últimos recados</b></h2><div class="col-12"><ul class="list-group shadow"><?
...
<span class="badge badge-danger badge-notification users-number" style="font-size: 10px"><a href="/paginas?a=excluircomentario&id=<?php echo $item->id; ?>"><font color="white">[excluir]</font></span></a>
<?php } ?> </div><?php $i++; } } else { ?>
...
?>
<div class="col-12 text-center" style="margin-top: 20px"><a href="/paginas.php"><?php echo $imgsetavoltar;?> Voltar</a><br/>
<a href="/home"><?php echo $imginicio;?>Página principal</a><br><br>
</div> </section><footer><?
}
else
...
?>
<section class="container-fluid">
<div id="titulo">Recados</div>
<div class="col-12">
<ul class="list-group shadow">
<li class="list-group-item">
<a href="paginas.php?"><?php echo $imgseta;?> Recados (<?php echo $contrec[0];?>)</a></li>
<li class="list-group-item"><a href="paginas.php?a=ultimos"><?php echo $imgseta;?> Recados recentes (<?php echo $contrec1[0];?>)</a></li></ul>
...
if(perm($meuid)>0){
?>
<br><form action="/paginas/emocoes/<?php echo $id;?>/<?php echo $pag;?>" method="post" enctype="multipart/form-data">
...
?>
<div align="center"><a href="/paginas/catemocoes"><?php echo $imgsetavoltar;?> Voltar</a><br/><br/></div>
<?
} 
...
<a href="/paginas/emocoes/<?php echo $item->id;?>"><?php echo $item->nm;?>(<?php echo $contemo2[0];?>)</a><br/>
</div><?php $i++; 
} 
if($numpag>1) {
paginas('paginas',$a,$numpag,$id,$pag);
...
?>
<section class="container-fluid"><h2 class="title"><b>Página</b></h2><div class="col-12"><ul class="list-group shadow"><?
...
?><img width="120" src="/<?php echo $dele['imagem']==false?'semfoto.jpg':''.$dele['imagem'].'';?>"><br/><em style="color: <?php echo $dele['cor']; ?>;background-color:<?php echo $dele['contorno']; ?>;"><?php echo textot($dele['txt'],$meuid,$on);?></em><br/><b>Dono(a): <a href="/<?php echo gerarlogin($dele['uid']);?>"><?php echo gerarnome($dele['uid']);?></a> -  <small><?php echo date("d/m - H:i:s", $dele['hora']);?></small><br/><strong>Postar recado</strong><br/>
<form action="/paginas?a=comentarios&id=<?php echo $id;?>" method="post" enctype="multipart/form-data">
