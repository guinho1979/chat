...
<header> <nav class="navbar navbar-light bg-light" style="margin-bottom: 20px"><a class="navbar-brand" href="#"><img src="/style/<?php echo $testearray[30];?>" width="32" height="" alt=""></a></header><?
...
?><li class="list-group-item"><div id="<?php echo $i%2==0?'div1':'div1';?>"><?if($conta[0]>0 or $schat->dn==$meuid){?><a style="font-size: 16px" href="/chat?a=sala&id=<?php echo $schat->id;?>"><i class="fas fa-lock-open" style="color: #B18904"></i><?php echo $schat->nm;?></span></a><?}else{?><i class="fas fa-lock" style="color: #00587C"></i> <?php echo $schat->nm;?></span><?}?></li><?
$i++;
}
...
?>
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-150381811-6"></script><script>  window.dataLayer = window.dataLayer || [];  function gtag(){dataLayer.push(arguments);}  gtag("js", new Date());  gtag("config", "UA-150381811-6");</script>
...
if($conteu[0]==0) { ?>
<br/><div align="center"><?php echo $imgerro;?>Esta sala está cheia, por favor escolha outra com menos de 50 usuários online<br/><br/> 
...
<br/><div align="center"><?php echo $imgerro;?>Somente é permitido usuários vip e membros da equipe nesta sala.<br/><br/>
<a href="chat"> <?php echo $imgbatepapo;?>
Salas de chat</a><br/>
<div class="col-12 text-center" style="margin-top: 20px"> <a href="javascript:window.history.back();"><?php echo $imgsetavoltar;?> Voltar</a></div><br/>
<?php exit(); } } else if($sperm[0]==2 and perm($meuid)==0 and $vip[1]==0) { ?>
<br/><div align="center"><?php echo $imgerro;?>Somente é permitido membros da equipe nesta sala.<br/><br/>
<a href="chat"> <?php echo $imgbatepapo;?>
Salas de chat</a><br/>
<div class="col-12 text-center" style="margin-top: 20px"> <a href="javascript:window.history.back();"><?php echo $imgsetavoltar;?> Voltar</a></div><br/>
<?php exit(); }
...
</div></div><div class="dropdown" style="display: inline-flex"><!--<button class="btn btn-secondary btn-micro dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fas fa-volume-up" style="color: #ffffff"></i></button><div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton"><a class="dropdown-item msg-audio" href="#" value="01">Vai a merda</a><a class="dropdown-item msg-audio" href="#" value="02">Tô cagando e andando pra vc</a><a class="dropdown-item msg-audio" href="#" value="03">Vai tomar no seu c*</a><a class="dropdown-item msg-audio" href="#" value="09">Fiu fiu</a><a class="dropdown-item msg-audio" href="#" value="04">Buzina</a><a class="dropdown-item msg-audio" href="#" value="05">Buzina 2</a><a class="dropdown-item msg-audio" href="#" value="07">Grito</a><a class="dropdown-item msg-audio" href="#" value="08">Tiro</a></div>--></div></div></div><div class="input-group input-group-sm mb-3 msg"><div class="input-group-prepend"><span class="input-group-text"><a href="/chat/catemocoes" class="smilies"><?php echo $imgsmilies;?></a></span></div><input type="text" class="form-control"  placeholder="Digite Sua Mensagem" id="caixatexto" name="msg" max-length="200"  class="inserirtexto" onfocus="Escrevendo();" onblur="ParouEscrever();" style=""><div class="input-group-append"><span class="input-group-text file-chooser"><a style="margin: 0 4px" data-toggle="collapse"href="#youtube"><i class="ion-social-youtube" style="color: #000000"></i></a></span></div><br><div class="input-group-append"><span class="input-group-text file-chooser"><a style="margin: 0 4px" data-toggle="collapse" href="#multimidia"><i class="fas fa-paperclip" style="color: #000000"></i></a></span><br><button class="btn btn-secondary btn-sm" name="envio" type="submit"><i class="fas fa-paper-plane"></i></button></div></div></div><div class="collapse" id="youtube" style="margin-bottom: 1rem"><input type="text" class="form-control form-control-sm" placeholder="Link do YouTube" name="youtube"></div><div class="collapse" id="multimidia" style="margin-bottom: 1rem"><br/><input type="file" name="arquivo" id="arquivo" /></div></div><input type="hidden" id="to" name="para" value="0"><input type="hidden" id="action" value="0" name="acoes"><input type="hidden" id="audio" value="0" name="audio"><div class="row"><div class="col-6 text-center" style="font-size: 16px; word-wrap: break-word"><span class="nick" name="dest" value="<?php echo $_GET["para"]==true ? "".$_GET["para"]."" : "0";?>"><?php echo $imgseta;?><?php echo $_GET["para"]==true ? "".gerarnome($_GET["para"])."" : "Todos";?></span></div><div class="col-6 text-center" style="font-size: 16px"><input type="hidden" name="dest" value="<?php echo $_GET["para"]==true ? "".$_GET["para"]."" : "0";?>"/><input type="checkbox" name="p" value="1"> Privada</div></div><center><div class="color-btn-wrapper"><button class="btn"><i class="bt_color"></i></button><input type="color" name="cortexto" value="#000000"></div><div class="color-btn-wrapper"><button class="btn"><i class="bt_color"></i></button><input type="color" name="contorno" value="#000000"></div></div></div></center></form>
...
<!-- MANTENHA FECHADO <script>function copiaemoji(str) {
    var texto = document.getElementById("caixatexto").value;
    document.getElementById("caixatexto").value = texto + '.' + str + '.';
}</script>
<style>
#pop{display:none;}
</style>
<a href="#" onclick="document.getElementById('pop').style.display='block';"><small>SMILIES</small></a>
<div id="pop">
<a href="#" onclick="document.getElementById('pop').style.display='none';"><small>[Fechar]</small></a>
<br />
<div style="width: -webkit-fill-available; margin: auto; text-align: center; overflow-x: auto; overflow-y: hidden; white-space: nowrap;"><?
//$itens = $mistake->query("SELECT id,cod,ext FROM w_emocoes  ORDER BY rand() LIMIT 200");	
//$i=0; while ($item = $itens->fetch(PDO::FETCH_OBJ)) {
//$item->cod = preg_replace("/[^a-zA-Z0-9]/", "", $item->cod);
?>
<div style="display: inline-block; position: relative; padding: 4px;">
<?php //echo $item->cod;?><br/><img onclick="copiaemoji('<?php //echo $item->cod;?>')" src="/e/<?php //echo $item->id;?>.<?php //echo $item->ext;?>" width="30" oncontextmenu='return false' onselectstart='return false' ondragstart='return false'>&nbsp;</div>|
<?php //$i++; }
?></div></div>-->
<style>
...
document.getElementById("voz").innerHTML = 'Gravando...<br><span id="clock"></span>';
var segundos = 0;
...
window.addEventListener('DOMContentLoaded', () => {EmojiButton(document.querySelector('.emoji-button'), function (emoji) {document.querySelector('input').value += emoji;});});
</script>
<center><div class="gravador-btn-wrapper"><button type="button" style="cursor :pointer" class="btn recordm"><i class="record"></i></i></button><div class="gravador-btn-wrapper"><button type="button" style="cursor :pointer" class="btn emoji-button"><i>&#128512;</i></button></div></div><br/>
...
if($bingo[0]==1){
?>
<center><div id="div2"><a href='bingowap?a=menu'><img src='imagens/bingo.gif' alt='img'/>Bingo<font color="green"> (Ativo valendo uma recarga ou prêmios) </font></a></div>
...
echo '<center>';
 $banner = array();
 $banner[] = '<a href="https://iptvsky.com.br"target="_blank"> <img src="/banner/banner1.png" width="320px" height="80px" /></a>';
 //$banner[] = '<a href="https://iptvsky.com.br"target="_blank"><img src="/banner/banner1.png" width="320px" height="80px" /></a>';
 //$banner[] = '<a href="https://iptvsky.com.br"target="_blank"><img src="/banner/banner1.png" width="320px" height="80px" /></a>';
 shuffle($banner);
 echo $banner[0];
